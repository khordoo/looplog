...
import type { Exercise, ExerciseLog, PerformanceRecord, SetLog, WorkoutSession } from '../../domain/types'
...
export function HistoryDetail() {
  const storage = useStorage(); const [session, setSession] = useState<WorkoutSession>(); const [logs, setLogs] = useState<ExerciseLog[]>([]); const [sets, setSets] = useState<SetLog[]>([]); const [previous, setPrevious] = useState<Record<string, PerformanceRecord[]>>({}); const [catalog, setCatalog] = useState<Exercise[]>([]); const { sessionId = '' } = useParams()
  useEffect(() => {
    let cancelled = false
    void Promise.all([storage.getSession(sessionId), storage.listExercises ? storage.listExercises({ includeArchived: true }) : Promise.resolve([] as Exercise[])]).then(async ([found, records]) => {
      if (!found) return
      ...
      if (!cancelled) { setSession(found); setLogs(nextLogs); setSets(nextSets); setPrevious(nextPrevious); setCatalog(records) }
    }).catch(() => undefined)
