  ...
  return <section className={`movement-preview${compact ? ' movement-preview-compact' : ''}${online && showVideo ? ' movement-preview-video' : ''}`} aria-label={`${exercise.name} visual guide`} data-testid="movement-preview">
    <div className="movement-media">
      {online && showVideo && exercise.media
        ...
        : art
          ? <img src={art} alt={`${exercise.name} setup illustration`} width="640" height="640" loading={compact ? 'eager' : 'lazy'} />
    ...
    <p className="visual-caption">{online ? 'Illustrated setup preview' : 'Video needs internet; the written guide remains available'} · Verified video by {exercise.media?.sourceName ?? 'the exercise source'}</p>
  </section>
}
