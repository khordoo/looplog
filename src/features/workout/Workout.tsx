...
export function Workout() {
  ...
  return <div className="page workout-page">
    ...
    <Card><h3>Form guide</h3>{exercise ? <><p><strong>Setup:</strong> {exercise.setup.join(' ')}</p><ol>{exercise.steps.map((step) => <li key={step}>{step}</li>)}</ol><p><strong>Tempo/breathing:</strong> {exercise.breathingTempo}</p><ul>{exercise.formCues.slice(0, 5).map((cue) => <li key={cue}>{cue}</li>)}</ul>{exercise.bandWarnings.length > 0 && <p className="help"><strong>Band warning:</strong> {exercise.bandWarnings.join(' ')}</p>}{exercise.media && <>{online && !showVideo && <Button variant="secondary" onClick={() => setShowVideo(true)} data-testid="load-video">Load demonstration</Button>}{online && showVideo && <div className="video-frame"><iframe title={exercise.media.title} src={`https://www.youtube-nocookie.com/embed/${exercise.media.videoId}`} loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /></div>}{!online && <Status kind="warning">This demonstration needs connectivity. The written guide remains available offline.</Status>}<p className="help">Demonstration by {exercise.media.sourceName} · <a href={exercise.media.sourceUrl} target="_blank" rel="noreferrer">Open on YouTube</a></p></>}</> : <p>Written guide unavailable for this exercise.</p>}</Card>
