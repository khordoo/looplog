...
export function Workout() {
  ...
  if (phase === 'warmup') return <div className="page workout-page"><UpdateNotice activeWorkout /><p className="eyebrow">Workout {session.workoutKey} · warm-up</p><h2>Prepare to move</h2><Card><h3>Four-minute warm-up</h3><p>March gently, rehearse a comfortable squat and hinge, move your shoulders, and breathe easily. Inspect your band before loading it.</p><ul><li>Easy march — 60 seconds</li><li>Comfortable bodyweight squat — 60 seconds</li><li>Shoulder-blade retractions and arm circles — 60 seconds</li><li>Relaxed hip-hinge rehearsal — 60 seconds</li></ul><Button onClick={() => setPhase('working')}>Start working sets</Button></Card></div>
  ...
  return <div className="page workout-page">
    ...
    {recommendation && <Status kind="info"><strong>Proposal:</strong> {targetLabel(recommendation.proposedTarget)} — {recommendation.rationale} <Button onClick={() => void applyRecommendation()} data-testid="confirm-recommendation">Confirm and use</Button></Status>}
    <Card><h3>Form guide</h3>{exercise ? <><p><strong>Setup:</strong> {exercise.setup.join(' ')}</p><ol>{exercise.steps.map((step) => <li key={step}>{step}</li>)}</ol><p><strong>Tempo/breathing:</strong> {exercise.breathingTempo}</p><ul>{exercise.formCues.slice(0, 5).map((cue) => <li key={cue}>{cue}</li>)}</ul>{exercise.bandWarnings.length > 0 && <p className="help"><strong>Band warning:</strong> {exercise.bandWarnings.join(' ')}</p>}{exercise.media && !showVideo && <Button variant="secondary" onClick={() => setShowVideo(true)} data-testid="load-video">Load demonstration</Button>}{exercise.media && showVideo && (online ? <div><iframe title={exercise.media.title} src={`https://www.youtube-nocookie.com/embed/${exercise.media.videoId}`} loading="lazy" allowFullScreen /><p className="help">{exercise.media.sourceName} · <a href={exercise.media.sourceUrl} target="_blank" rel="noreferrer">Open on YouTube</a></p></div> : <Status kind="warning">This demonstration needs connectivity. The written guide remains available offline.</Status>)}</> : <p>Written guide unavailable for this exercise.</p>}</Card>
    <Card><h3>{currentSets.length >= expectedSets ? `Target sets saved (${expectedSets} of ${expectedSets})` : `Set ${currentSets.length + 1} of ${expectedSets}`}</h3><div className="two-col"><label>{isTimed ? 'Duration (seconds)' : 'Repetitions'}<input inputMode="numeric" type="number" min="1" value={isTimed ? duration : reps} onChange={(event) => isTimed ? setDuration(event.target.value) : setReps(event.target.value)} data-testid={isTimed ? 'set-duration' : 'set-reps'} /></label><label>Effort<select value={effort} onChange={(event) => setEffort(event.target.value as EffortRating)} data-testid="effort-select">{effortLabels.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><fieldset><legend>Band or bodyweight</legend><div className="band-choices">{bands.map((band) => <label key={band.key}><input type="checkbox" checked={bandKeys.includes(band.key)} onChange={() => setBandKeys((current) => current.includes(band.key) ? current.filter((item) => item !== band.key) : [...current, band.key])} /> <span className={`band-dot band-${band.displayColor}`} aria-hidden="true" />{displayBand(band)}</label>)}<span className="help">Leave all unchecked for bodyweight. Multiple bands may be selected.</span></div></fieldset><label>Setup adjustment<select value={setup} onChange={(event) => setSetup(event.target.value as SetupAdjustment)} data-testid="setup-adjustment">{adjustmentOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>{setup === 'other' && <label>Describe setup<input value={setupNote} onChange={(event) => setSetupNote(event.target.value)} placeholder="e.g. shortened grip above the knees" data-testid="setup-note" /></label>}<label>Exercise notes<textarea value={exerciseNote} onChange={(event) => setExerciseNote(event.target.value)} onBlur={() => void saveExerciseNote()} placeholder="Optional notes" data-testid="exercise-note" /></label><Button onClick={() => void completeSet()} disabled={currentSets.length >= expectedSets} data-testid="set-complete">{currentSets.length >= expectedSets ? 'Target sets saved' : 'Save set'}</Button></Card>
